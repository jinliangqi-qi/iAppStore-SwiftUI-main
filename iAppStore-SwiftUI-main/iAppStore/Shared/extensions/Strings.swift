...
extension String {
    ...
    /// - Returns: 缩略图 Size
    public func imageAppleSize() -> Size {
        if let url = URL(string: self) {
            ...
            if splits.count > 1, let fSize = splits.first, let lSize = splits.dropFirst().first {
                if let fdSize = Double(fSize), let ldSize = Double(lSize) {
                    return Size(width: fdSize, height: ldSize)
                }
            }
        }
        return Size.zero
    ...
    /// - Returns: 缩放后的 url
    @MainActor
    public func imageAppleScale(_ scale: Double? = nil) -> String {
        let actualScale = scale ?? UIScreen.main.scale
        if let url = URL(string: self) {
            let component = url.lastPathComponent
            let size = imageAppleSize()
            let newfSize = String(Int(size.width * actualScale))
            let newlSize = String(Int(size.height * actualScale))
